...
async function fetchWeeklyContent(effectiveDate: string): Promise<WeeklyContent | null> {
  ...
  const { data, error } = await supabase
    .from("content")
    .select(
      "id, title, description, excerpt, featured_image_url, thumbnail_image_url, body, reflection_prompt, download_url, resource_links, vimeo_video_id, youtube_video_id, castos_episode_url, s3_audio_key, duration_seconds, week_start"
    )
