...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .eq("kind", "image")
    .eq("usage_context", "content")
    .eq("status", "active")
    .order("created_at", { ascending: false })
    .limit(80);

  if (error) {
    console.error("[content-images] library lookup failed:", error.message);
...
export async function POST(request: Request) {
  ...
  if (error || !data) {
    console.error("[content-images] media metadata insert failed:", error?.message);
