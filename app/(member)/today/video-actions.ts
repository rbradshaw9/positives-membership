 ...
 * and resume-from-last-position. One row per (user, content_id) or
 * (user, mux_asset_id) when no content_id is available.
 *
...
export async function recordVideoProgress({
  ...
  const lookupFilter = contentId
    ? { user_id: user.id, content_id: contentId }
    : { user_id: user.id, mux_asset_id: muxAssetId! };
...
export async function getVideoResumePosition({
  ...
  if (!contentId && !muxAssetId) return 0;
